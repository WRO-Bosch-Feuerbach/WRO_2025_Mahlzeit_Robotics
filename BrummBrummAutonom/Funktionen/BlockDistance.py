...
def berechne_entfernung(hohe_im_bild, mittlere_brennweite, echte_hoehe=100):
    """
    Berechnet die Entfernung basierend auf der Blockhoehe im Bild und der mittleren Brennweite.
    
    Parameters:
    hohe_im_bild (int): Hoehe des Blocks im Bild (in Pixel).
    mittlere_brennweite (float): Mittlere Brennweite (in Pixel).
    echte_hoehe (float): Echte Hoehe des Blocks (in mm), Standardwert ist 100 mm.
...
# Farbbereiche für Rot und Gruen im HSV-Raum
...
def erkenne_block(im_bild):
    """
    Erkennen des Blocks im Bild und Ermitteln der Hoehe des Blocks in Pixel.
    ...
    Returns:
    int: Hoehe des Blocks im Bild (in Pixel).
    """
    ...
    if konturen:
        ...
        return h  # Hoehe des Blocks im Bild (in Pixel)
...
def bestimme_farbe(im_bild):
    ...
    Returns:
    str: Die erkannte Farbe des Blocks ("rot", "gruen" oder "keine").
    """
    hsv = cv2.cvtColor(im_bild, cv2.COLOR_BGR2HSV)

    # Erstelle Masken für Rot und Gruen
    maske_rot = cv2.inRange(hsv, red_lower, red_upper)
    maske_gruen = cv2.inRange(hsv, green_lower, green_upper)

    # Zaehle die Pixel in jeder Maske
